...
class App extends Component {
  state = {
    ...
    addBook() {
      ...
      axios.post('http://localhost:8000/movies/',  this.state.newBookData,this.state.headerInfo).then((response) => {
    ...
    updateBook() {
      ...
      axios.put('http://localhost:8000/movies/' + this.state.editBookData.id+'/', {
        title, desc, year
    ...
    deleteBook(id) {
      axios.delete('http://localhost:8000/movies/' + id+'/').then((response) => {
        this._refreshBooks();
      });
    }
